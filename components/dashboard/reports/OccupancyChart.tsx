    ...
    <div className="xl:col-span-2 bg-white rounded-2xl border border-ink-100 shadow-[0_1px_2px_0_rgba(15,23,42,0.04)] p-5">
      <div className="flex items-center justify-between mb-4 flex-wrap gap-2">
        <div>
          ...
          <p className="text-xs text-ink-500 mt-0.5">Son 12 ay · % bazında</p>
        </div>
        <div className="flex gap-2 text-xs font-semibold">
          {['12 Ay', '6 Ay', '30 Gün'].map((v, i) => (
            <button key={v} className={`px-3 py-1.5 rounded-md transition ${i === 0 ? 'bg-ink-900 text-white' : 'text-ink-500 hover:bg-ink-50'}`}>{v}</button>
          ))}
        </div>
      </div>
      ...
      <div className="relative h-48 sm:h-56">
        ...
        <svg viewBox="0 0 400 200" className="absolute inset-0 w-full h-full" preserveAspectRatio="none">
          ...
          <path d="M0,150 L33,130 L66,140 L100,100 L133,110 L166,80 L200,90 L233,60 L266,70 L300,50 L333,40 L366,30 L400,26 L400,200 L0,200 Z" fill="url(#fillGrad)" />
          <path d="M0,150 L33,130 L66,140 L100,100 L133,110 L166,80 L200,90 L233,60 L266,70 L300,50 L333,40 L366,30 L400,26" fill="none" stroke="#4f46e5" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" />
          <circle cx="400" cy="26" r="5" fill="#4f46e5" />
          <circle cx="400" cy="26" r="9" fill="#4f46e5" fillOpacity="0.2" />
        </svg>
      </div>
