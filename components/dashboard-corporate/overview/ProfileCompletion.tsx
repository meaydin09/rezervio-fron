...
interface Props {
  onClose: () => void
  onNavigate: (view: CorporateView) => void
}

const completed = ['Firma Bilgileri', 'Logo & Marka', 'Adres & İletişim', 'Uzmanlar (12)']
const pending: { label: string; view: CorporateView }[] = [
  { label: 'Hizmet Açıklamaları', view: 'services' },
  { label: 'Klinik Galerisi',     view: 'settings'  },
]
      ...
      <div className="relative flex items-start gap-4 flex-wrap">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 flex-wrap">
            <h3 className="font-bold text-base sm:text-lg text-ink-900">Kurumsal profilinizi güçlendirin</h3>
            <span className="inline-flex items-center text-[11px] font-bold px-2 py-0.5 rounded-full bg-white text-brand-700 border border-brand-200">%72</span>
          </div>
          <p className="text-xs sm:text-sm text-ink-600 mt-1">
            Hizmetlerinizi detaylı tanıtarak ve klinik fotoğraflarınızı ekleyerek{' '}
            <strong className="text-ink-900">3.1 kat</strong> daha fazla randevu çekebilirsiniz!
          </p>
          <div className="mt-3 h-2 bg-white rounded-full overflow-hidden shadow-inner">
            <div className="h-full bg-gradient-to-r from-brand-500 via-violet-500 to-rose-500 rounded-full" style={{ width: '72%' }} />
          </div>
          <div className="mt-4 grid sm:grid-cols-2 lg:grid-cols-3 gap-2">
            ...
            {pending.map((item) => (
              <button key={item.label} onClick={() => onNavigate(item.view)} className="flex items-center gap-2 p-2 rounded-lg bg-amber-50/80 border border-amber-200 hover:bg-amber-100 transition cursor-pointer text-left">
