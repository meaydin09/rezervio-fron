...
import { Plus, X, Check, ChevronRight, Calendar, MessageCircle, Palette, CreditCard, Users, Send, Upload } from 'lucide-react'
...
const tickets: Ticket[] = [
  ...
  },
  {
    ...
    previewAuthor: 'Destek Ekibi', previewAuthorColor: 'text-brand-700',
    meta: '3 gün önce kapatıldı', priority: 'Normal öncelik', rating: '⭐⭐⭐⭐⭐ Değerlendirildi',
  },
]
    ...
    <div className="grid xl:grid-cols-3 gap-6">
      ...
      <div className="xl:col-span-2 bg-white rounded-2xl border border-ink-100 shadow-[0_1px_2px_0_rgba(15,23,42,0.04)] p-5">
        ...
        <div className="space-y-2.5">
          {tickets.map((t) => (
            <div key={t.id} className="p-4 rounded-xl border border-ink-100 hover:bg-ink-50/50 transition cursor-pointer">
              <div className="flex items-start justify-between gap-3 flex-wrap">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 flex-wrap mb-1">
                    <span className="text-xs font-mono text-ink-400">{t.id}</span>
                    <span className="text-[11px] font-medium text-ink-500">{t.priority}</span>
                  </div>
        ...
        <div className="fixed inset-0 z-50 bg-ink-900/50 backdrop-blur-sm flex items-center justify-center p-4 overflow-y-auto">
          <div className="bg-white rounded-2xl shadow-[0_10px_30px_-10px_rgba(79,70,229,0.25)] max-w-lg w-full">
            <div className="p-5 border-b border-ink-100 flex items-center justify-between">
              ...
              <button onClick={() => setShowModal(false)} className="w-8 h-8 rounded-lg hover:bg-ink-100 flex items-center justify-center cursor-pointer transition">
                <X className="w-4 h-4 text-ink-500" strokeWidth={2} />
              </button>
            </div>

            <form onSubmit={(e) => { e.preventDefault(); setShowModal(false) }} className="p-5 sm:p-6 space-y-4">
              <div>
              ...
              <div className="flex items-center justify-end gap-2 pt-2 border-t border-ink-100">
                <button type="button" onClick={() => setShowModal(false)} className="px-4 py-2 text-sm font-semibold bg-ink-100 hover:bg-ink-200 text-ink-800 rounded-lg cursor-pointer transition">
                  İptal
                </button>
